...
class GPT2CustomLMHeadModel(GPT2LMHeadModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # For Attribute-Class deduction dataset, the word size of an <attribute> or <class> may be different from the x_word size
            k = (attribute_class_word_size or word_size)                        # if attribute_class_word_size=0, then k is set to word_size
            if loss_last_word:
                # select logits and labels for last k tokens corresponding to last word 
                # i.e. (x1,... xk) --> (y1,... yk)
                last_word_logits = shift_logits[..., -k:, :].contiguous()       # skips first (k-1) tokens + arrow
                last_word_labels = shift_labels[..., -k:].contiguous()          # only take last k tokens (last word)
                # print("last word labels:", last_word_labels.size())
                # print("last_word_logits:", last_word_logits.size())

            else:
                last_word_logits = shift_logits     
                last_word_labels = shift_labels

            # Flatten the tokens
            flattened_logits = last_word_logits.view(-1, last_word_logits.size(-1)) # batch x |V|
            flattened_labels = last_word_labels.view(-1)                            # batch
            # print("flattened_logits:", flattened_logits.size())
            # print("flattened_labels:", flattened_labels.size())
            loss = loss_fct(flattened_logits, flattened_labels)

            # softmax into probability vector
            if loss_last_word:
                batch_probs_vec = F.softmax(last_word_logits, dim=-1).detach()              # last_word_logits: batch x word_size x |V|, softmax along |V| dim
            else:
                # FIXME: do a better fix
                batch_probs_vec = F.softmax(last_word_logits[..., -k:, :], dim=-1).detach()   # needs to skip arrow, when case loss_last_word = False
                last_word_labels = last_word_labels[..., -k:]
            # print("batch_probs_vec:", batch_probs_vec.size())                  # batch_probs_vec: batch x word_size x |V|
            last_word_probs = []
        ...
        out_dict = dict(
            last_word_probs=last_word_probs, 
            last_word_first_token_probs=last_word_first_token_probs, 
            token_0_prob=token_0_prob, 
            token_1_prob=token_1_prob,
        )

        if return_logits:
            '''
            lm_logits:                 N x L x |V|              # L = original sequence length (word1 -> word2)
            shift_logits:           N x (L-1) x |V|
            last_word_logits:       N x k x |V|                 # k = word length
            flattened_logits:       N * k x |V|                 # same as batch x |V| if word size k = 1
            flattened_lables:       length N*k
            
            flattened_logits:                       # logits vector for 3rd token predicton for each sentence in batch
            [ 
                1: [------- |V| --------]
                            ...
                N: [------- |V| --------]
            ...
            flattened_labels:                       # target 3rd token id for each sentence in batch
            [
                1: y1
                   ...
                N: yn
            ...
            '''
            # For visualizing logits, we are passing in 'AB' as input to the transformer rather than 'A-->B'
            # But last_word_logits = shift_logits[:, -k:, :] so last_word_logits == shift_logits (since there's not 0 or 1 to skip)
            out_dict['logits'] = flattened_logits
